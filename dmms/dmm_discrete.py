        ...
        with pyro.plate("z_minibatch", len(x_batch)):
            ...
            for t in pyro.markov(range(T_max)):
                ...
                if t == 0:
                    ...
                    b_tilde_t = self.trans(b_tilde_prev, a_batch[:, t]) # if this doesn't work, maybe passing z_prev as input?
                # track variables
                b_tilde_t = pyro.deterministic("b_tilde_%d" % t, b_tilde_t)
                ...
                if self.use_action_emitter:
                    emission_probs_t = self.emitter(z_t, a_batch[:, t])
                else:
        ...
        with pyro.plate("z_minibatch", len(x_batch)):
            ...
            for t in pyro.markov(range(T_max)):
                ...
                if t == 0:
                    ...
                    b_tilde_t = self.trans(b_prev, a_batch[:, t]) # if sharing parameters doesn't work,
                    ...
                    b_t = b_tilde_t = self.trans(b_prev, a_batch[:, t])
                # track variables
